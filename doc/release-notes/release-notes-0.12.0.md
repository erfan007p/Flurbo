Flurbo Core version 0.12.0 is now available from:

  <https://flurbo.org/bin/flurbo-core-0.12.0/>
...
backwards-compatible with pre-0.10 versions of Flurbo Core or other software:
...
ECDSA signatures inside Flurbo transactions now use validation using
[libsecp256k1](https://github.com/flurbo-core/secp256k1) instead of OpenSSL.
...
(https://github.com/flurbo/bips/blob/master/bip-0130.mediawiki)
...
Previous versions of Flurbo Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Flurbo Core was to raise the default minimum
relay fee.

Flurbo Core 0.12 will have a strict maximum size on the mempool. The
...
Flurbo Core 0.12 also introduces new default policy limits on the length and
...
Flurbo Core 0.12 nodes. Flurbo Core will only allow replacement of
transactions which have any of their inputs' `nSequence` number set to less
than `0xffffffff - 1`.  Moreover, a replacement transaction may only be
accepted when it pays sufficient fee, as described in [BIP 125]
(https://github.com/flurbo/bips/blob/master/bip-0125.mediawiki).
...
Note that the wallet in Flurbo Core 0.12 does not yet have support for
...
This allows running flurbod without having to do any manual configuration.
...
Flurbo Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Flurbo Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In Flurbo Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in Flurbo Core 0.13, and it is at this time undecided whether the
...
Flurbo Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Flurbo Core automatically creates a hidden service to listen on, without
manual configuration. Flurbo Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Flurbo Core is listening, and
...
Flurbod can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Flurbo
Core to use floating fees. By default, Flurbo Core will use floating
fees.
...
At all times, Flurbo Core will cap fees at `-maxtxfee=<x>` (default:
0.10) BTC.
Furthermore, Flurbo Core will never create transactions paying less than
...
`flurbo.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of v0.11.0](https://github.com/flurbo/flurbo/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/flurbo/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/flurbo/bips/blob/master/bip-0065.mediawiki)
...
caching. A sample config for apache2 could look like:
    ...
    <Location /flurborpc>
        ...
        # optional bypass flurbod rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< flurborpc:<password>
    </Location>
...
Restarting flurbod will no longer clear out the list of banned peers; instead
...
- #6214 `87406aa` [OSX] revert renaming of Flurbo-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new flurbo-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include flurbo-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix flurbo-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of flurbo-cli on secp256k1 (Wladimir J. van der Laan)
...
- #7209 `f3ad812` test: don't override FLURBOD and FLURBOCLI if they're set (Wladimir J. van der Laan)
...
- #6387 `11576a5` [flurbo-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add FLURBOD_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libflurboconsensus (Braydon Fuller)
