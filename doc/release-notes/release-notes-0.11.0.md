Flurbo Core version 0.11.0 is now available from:

  <https://flurbo.org/bin/flurbo-core-0.11.0/>
...
For example, add the following to `flurbo.conf`:
...
related to the blockchain in the flurbo system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Flurbo Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Flurbo Core will 
...
For an example script see [flurbo-submittx](https://github.com/laanwj/flurbo-submittx).
...
- #5334 `cf87536` libflurboconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new flurbo-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only FlurboAmountField
- #5753 `0f44672` Add flurbo logo to about screen
...
- #5809 `46bfbe7` Add flurbo-cli man page
...
- #5849 `41259ca` contrib/init/flurbod.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Flurbo-Qt.app to Flurbo-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for flurbo-tx)
