...
###   This script attempts to download the signature file SHA256SUMS.asc from flurbo.org
...
WORKINGDIR="/tmp/flurbo"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://flurbo.org/bin/"
VERSIONPREFIX="flurbo-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'flurbo-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Flurbo Core binary release signing key installed?"
   fi
